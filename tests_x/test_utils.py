...
def test_get_completion(mock_openai_client):
    mock_openai_client.chat.completions.create.return_value.choices[
        0
    ].message.content = "Mocked completion"
...
def test_get_completion_from_messages(mock_openai_client):
    mock_openai_client.chat.completions.create.return_value.choices[
        0
    ].message.content = "Mocked completion from messages"
...
def test_zero_shot(mock_openai_client):
    mock_openai_client.chat.completions.create.return_value.choices[
        0
    ].message.content = "Mocked zero-shot response"
    ...
    mock_openai_client.chat.completions.create.assert_called_once_with(
        ...
        messages=[
            ...
            },  ## erwartet die system message, die eine lokale variable ist
            {
                "role": "user",
                "content": prompt,
            },  ## erwartet die prompt, die eine lokale Variable ist
        ],
        temperature=0,
        max_tokens=NOT_GIVEN,
        top_p=NOT_GIVEN,
